...
- NEW: RepresentingTextLesson - Learn about Unicode, UTF-8, encoding/decoding
- NEW: ListGamesLesson - Interactive dungeon crawler mini-game using lists
- ENHANCED: Deeper D&D storyline integration throughout Story Mode
- ENHANCED: Act transitions explain the "Lost Language of Nature" narrative
"""
...
class LessonFactory:
    ...
    def create_lesson(topic_id: str) -> Optional[Lesson]:
        ...
        lesson_map = {
            ...
            "representing_text": RepresentingTextLesson,
...
class RepresentingTextLesson(Lesson):
    """Lesson: Representing Text - Unicode, UTF-8, and Character Encoding"""

    def __init__(self):
        super().__init__(
            lesson_id="representing_text",
            title="Representing Text: Unicode and Encoding",
            description="Learn how computers represent text using Unicode, UTF-8, ASCII, and character encoding.",
            topic_id="representing_text"
        )

    def teach(self):
        print("""
Elder Willowbyte traces glowing symbols in the air - strange runes from
distant lands, each one shimmering with its own unique light...

'Observe, young one. These are not just symbols - they are GLYPHS from
every language, every culture, every writing system known to this world.'

═══════════════════════════════════════════════════════════════════════════
              REPRESENTING TEXT: THE LANGUAGE OF GLYPHS
═══════════════════════════════════════════════════════════════════════════

THE PROBLEM: How does a computer store text?

Computers only understand numbers (binary). So every character must be
assigned a NUMBER. This mapping is called CHARACTER ENCODING.

═══════════════════════════════════════════════════════════════════════════
1. ASCII - THE OLD WAY (7-bit, 128 characters)
═══════════════════════════════════════════════════════════════════════════

In the ancient times (1960s), ASCII was born:
- A-Z, a-z, 0-9, punctuation, control characters
- Each character = a number from 0-127
- Example: 'A' = 65, 'a' = 97, '0' = 48

    # Python's ord() gets the number for a character
    print(ord('A'))      # 65
    print(ord('Z'))      # 90
    print(ord('a'))      # 97
    print(ord('0'))      # 48

    # Python's chr() gets the character for a number
    print(chr(65))       # 'A'
    print(chr(97))       # 'a'
    print(chr(33))       # '!'

LIMITATION: ASCII only supports English! No é, ñ, 中, 日, 한, Ω, 🐉...

═══════════════════════════════════════════════════════════════════════════
2. UNICODE - THE UNIVERSAL SOLUTION
═══════════════════════════════════════════════════════════════════════════
...
'Unicode contains over 140,000 characters! Every language, every symbol,
unified into one great system. This is the true Language of Nature.'

═══════════════════════════════════════════════════════════════════════════
3. UTF-8 - HOW UNICODE IS STORED
═══════════════════════════════════════════════════════════════════════════

Unicode defines WHAT characters exist. UTF-8 defines HOW to store them
as bytes (variable-length encoding):

- ASCII characters (A-Z, 0-9): 1 byte
- European characters (é, ñ, ö): 2 bytes
- Asian characters (中, 日, 한): 3 bytes
- Emoji (🐉, 😀): 4 bytes

UTF-8 is the MOST COMMON encoding on the web (over 95% of websites).

═══════════════════════════════════════════════════════════════════════════
4. ENCODING AND DECODING IN PYTHON
═══════════════════════════════════════════════════════════════════════════
...
WHY THIS MATTERS:
- Reading files from disk: bytes → string (decode)
- Writing files to disk: string → bytes (encode)
- Network communication: must encode/decode
- Handling international text properly

═══════════════════════════════════════════════════════════════════════════
5. COMMON ENCODINGS
═══════════════════════════════════════════════════════════════════════════

    utf-8       Most common, supports all Unicode (RECOMMENDED)
    ascii       Only A-Z, a-z, 0-9 (7-bit, 128 chars)
    latin-1     Western European languages (8-bit, 256 chars)
    utf-16      Used by Windows, Java (2-4 bytes per char)
...
print(f"Code point: {ord(char)}")  # 128009
print(f"Hex: {hex(ord(char))}")    # 0x1f409
...
# Working with bytes
message = "Dragon's Lair: 龍"
bytes_utf8 = message.encode('utf-8')
bytes_ascii = message.encode('ascii', errors='ignore')  # Strips non-ASCII

print(bytes_utf8)   # b"Dragon's Lair: \\xe9\\xbe\\x8d"
print(bytes_ascii)  # b"Dragon's Lair: "

# Reading files with proper encoding
with open('spell_book.txt', 'r', encoding='utf-8') as f:
    content = f.read()  # Properly decoded!

═══════════════════════════════════════════════════════════════════════════
KEY TAKEAWAYS
═══════════════════════════════════════════════════════════════════════════

✓ ord(char) → Get numeric code point of a character
✓ chr(num) → Get character from numeric code point
✓ str.encode('utf-8') → Convert string to bytes
✓ bytes.decode('utf-8') → Convert bytes to string
✓ Python 3 strings are Unicode by default
✓ Always specify encoding='utf-8' when reading/writing files
✓ UTF-8 is the standard for web and modern applications

Elder Willowbyte's bark glows with ancient runes:

'Now you understand how the glyphs themselves are stored - not as images,
but as numbers. Every character in every language has its place in the
great Unicode table. This knowledge will serve you well when handling
text from all corners of Fraylon!'
        """)

    def challenge(self) -> bool:
        challenge = CodeChallenge(
            prompt="""Practice with character encoding:

1. Create a variable 'dragon_char' with the dragon emoji: 🐉
2. Get its Unicode code point using ord() and store in 'dragon_code'
3. Convert it back using chr() and store in 'dragon_back'
4. Encode the string "Hello" to UTF-8 bytes and store in 'encoded'

Example:
    dragon_char = '🐉'
    dragon_code = ord(dragon_char)
    dragon_back = chr(dragon_code)
    encoded = "Hello".encode('utf-8')
""",
            test_cases=[
                {'type': 'variable', 'variable': 'dragon_char', 'expected': '🐉'},
                {'type': 'variable', 'variable': 'dragon_code', 'expected': 128009},
                {'type': 'variable', 'variable': 'dragon_back', 'expected': '🐉'},
                {'type': 'variable', 'variable': 'encoded', 'expected': b'Hello'}
            ],
            hints=[
                "dragon_char = '🐉' (copy the emoji)",
                "dragon_code = ord(dragon_char)",
                "dragon_back = chr(dragon_code)",
                "encoded = \"Hello\".encode('utf-8')"
            ]
...
class ListGamesLesson(Lesson):
    """Lesson: Interactive List-Based Dungeon Crawler Game"""

    def __init__(self):
        super().__init__(
            lesson_id="list_games",
            title="List-Based Games: Dungeon Crawler",
            description="Learn list manipulation through an interactive dungeon crawler mini-game.",
            topic_id="list_games"
        )

    def teach(self):
        print("""
Elder Willowbyte's eyes gleam with excitement. 'Enough theory, young one!
Let us put your knowledge of LISTS to practical use. I shall teach you
the ancient art of dungeon exploration using list-based game logic!'

═══════════════════════════════════════════════════════════════════════════
                    THE CORRUPTED CATACOMBS - A LIST ADVENTURE
═══════════════════════════════════════════════════════════════════════════

'Behold!' The elder gestures, and a glowing dungeon map appears:

Lists are PERFECT for managing game state:
- Player inventory: list of items
- Dungeon rooms: list of room descriptions
- Enemy positions: list of coordinates
- Quest objectives: list of tasks

Let me show you how REAL games use lists...

═══════════════════════════════════════════════════════════════════════════
GAME CONCEPT: DUNGEON CRAWLER
═══════════════════════════════════════════════════════════════════════════

You'll navigate through dungeon rooms (stored in a list), manage your
inventory (another list), and battle enemies (you guessed it - lists!).

CORE MECHANICS USING LISTS:

1. INVENTORY MANAGEMENT
   inventory = []  # Empty backpack
   inventory.append('Health Potion')  # Pick up item
   inventory.append('Rusty Sword')
   inventory.remove('Rusty Sword')    # Drop/use item

   if 'Health Potion' in inventory:
       print("You have healing!")
...
2. ROOM NAVIGATION
   ...
   current_room = 0  # Start at index 0
   current_room += 1  # Move forward

   print(f"You are in: {rooms[current_room]}")
...
3. ENEMY ENCOUNTERS
   ...
   if len(enemies) > 0:
       ...
       print(f"A {current_enemy} attacks!")
       enemies.pop(0)  # Remove defeated enemy

4. LOOT COLLECTION
   loot = ['Gold Coin', 'Magic Ring', 'Health Potion']

   # Take all loot
   for item in loot:
       inventory.append(item)
   loot.clear()  # Empty the chest

═══════════════════════════════════════════════════════════════════════════
ADVANCED TECHNIQUES
═══════════════════════════════════════════════════════════════════════════

PARTY MANAGEMENT (List of Dictionaries):
    party = [
        {'name': 'Grixle', 'health': 100, 'class': 'Druid'},
        {'name': 'Thorin', 'health': 120, 'class': 'Warrior'},
        {'name': 'Elara', 'health': 80, 'class': 'Mage'}
    ]

    # Check who's alive
    alive = [hero for hero in party if hero['health'] > 0]

MAP GENERATION (2D List):
    dungeon_map = [
        ['#', '#', '#', '#', '#'],
        ['#', '.', '.', '.', '#'],
        ['#', '.', 'E', '.', '#'],  # E = Enemy
    ...
    # Move player
    dungeon_map[player_y][player_x] = '.'  # Clear old position
    player_x += 1  # Move right
    dungeon_map[player_y][player_x] = 'P'  # Place player

QUEST TRACKING:
    quests = [
        {'name': 'Defeat 5 Skeletons', 'progress': 3, 'goal': 5},
        {'name': 'Collect 3 Gems', 'progress': 1, 'goal': 3}
    ]

    for quest in quests:
        if quest['progress'] >= quest['goal']:
            print(f"✓ {quest['name']} - COMPLETE!")
        else:
            print(f"○ {quest['name']} - {quest['progress']}/{quest['goal']}")

═══════════════════════════════════════════════════════════════════════════
WHY LISTS ARE PERFECT FOR GAMES
═══════════════════════════════════════════════════════════════════════════

✓ Dynamic size - add/remove items as player progresses
✓ Ordered - maintain sequence of rooms, events, turns
✓ Indexed access - get specific room, enemy, or item quickly
✓ Iterable - loop through all enemies, items, or quests
✓ Flexible - can store any data type (strings, numbers, objects)

Elder Willowbyte nods with satisfaction:

'Now you see how the simple LIST becomes the foundation for complex
game systems. Every RPG, every roguelike, every adventure game uses
these exact techniques. You are ready to build your own!'
        """)

    def challenge(self) -> bool:
        """Interactive dungeon crawler mini-game"""
        print("""
═══════════════════════════════════════════════════════════════════════════
                    CHALLENGE: THE CORRUPTED CATACOMBS
═══════════════════════════════════════════════════════════════════════════

Welcome, brave adventurer! You must navigate through the Corrupted
Catacombs, manage your inventory, defeat enemies, and find treasure!

This is a PLAYABLE mini-game demonstrating list operations.
        """)

        # Game state (all using lists!)
        rooms = [
            "🚪 Dark Entrance - A musty smell fills the air",
            "💀 Skeleton Chamber - Bones rattle in the shadows",
            "🗡️  Armory - Rusty weapons hang on the walls",
            "👑 Treasure Room - Gold glimmers in the torchlight",
            "🐉 Dragon's Lair - The Iron Wyrm's corruption is strong here"
        ]

        inventory = ['Wooden Staff', 'Health Potion']
        enemies = ['Corrupted Goblin', 'Skeleton Warrior', 'Dark Mage']
        treasure_items = ['Gold Coin', 'Ancient Scroll', 'Magic Ring']

        current_room = 0
        player_health = 100
        enemies_defeated = 0

        print(f"\n🎒 Starting Inventory: {', '.join(inventory)}")
        ...
        while current_room < len(rooms):
            ...
            print("═" * 70)
            print(f"\nROOM {current_room + 1}/{len(rooms)}: {rooms[current_room]}")
            print(f"🎒 Inventory ({len(inventory)} items): {', '.join(inventory)}")
            print(f"❤️  Health: {player_health}")
            print()

            # Random encounter in middle rooms
            if 0 < current_room < len(rooms) - 1 and enemies:
                enemy = enemies.pop(0)
                print(f"⚔️  A {enemy} appears!")

                action = input("Fight (f) or use Health Potion (h)? ").strip().lower()

                if action == 'h' and 'Health Potion' in inventory:
                    inventory.remove('Health Potion')
                    player_health = min(100, player_health + 30)
                    print(f"💚 Used Health Potion! Health: {player_health}")
                    print(f"⚔️  You must still fight {enemy}!")
                    player_health -= 15
                    print(f"💔 Took 15 damage! Health: {player_health}")
                    enemies_defeated += 1
                elif action == 'f':
                    damage = random.randint(10, 25)
                    player_health -= damage
                    print(f"⚔️  You defeat {enemy}!")
                    print(f"💔 Took {damage} damage! Health: {player_health}")
                    enemies_defeated += 1
                else:
                    print("⚔️  You attack with your staff!")
                    player_health -= 20
                    print(f"💔 Took 20 damage! Health: {player_health}")
                    enemies_defeated += 1
            ...
            # Treasure in certain rooms
            if current_room == 3 and treasure_items:
                print(f"\n✨ You found treasure!")
                item = treasure_items.pop(0)
                inventory.append(item)
...
You have navigated the Corrupted Catacombs successfully!

Through this mini-game, you practiced:
✓ List indexing - accessing rooms[current_room]
✓ List methods - append(), remove(), pop()
✓ List membership - 'item' in inventory
✓ List length - len(inventory), len(enemies)
✓ List iteration - managing game state

Elder Willowbyte appears in a flash of light:

'Excellent work! You've seen how lists power real game mechanics. Every
inventory system, every level progression, every enemy encounter - all
built on the humble list. Go forth and create your own adventures!'
                """)
                print(f"\n📊 Final Stats:")
                print(f"   Rooms Explored: {current_room + 1}/{len(rooms)}")
                print(f"   Enemies Defeated: {enemies_defeated}")
        ...
        input("\n[Press Enter to complete the lesson...]")
        return True
...
class StoryMode:
    ...
    def _build_acts(self) -> Dict[int, Dict]:
        ...
        acts = {
            1: {
                "title": "Act I: The Ancient Glyphs",
                "intro": """
═══════════════════════════════════════════════════════════════════════════
...
syntax. Elder Willowbyte, a wise treant with bark like ancient parchment,
calls upon you to learn the Language of Nature - Python itself.

'Young Grixle,' the elder's voice resonates like wind through leaves,
'long ago, our world of Fraylon was built upon an ancient language -
a tongue spoken by the very trees, rivers, and stones. We called it
the Language of Nature. You know it as... Python.'

The elder's branches gesture to glowing runes floating in the air:

'But the Cult of the Dragon seeks to corrupt this language, twisting
its elegant syntax into chaotic spells. The Iron Wyrm, their master,
feeds on computational chaos. You must learn the fundamentals - the
ANCIENT GLYPHS - before the corruption spreads too far.'

'Start with the basics, young one. Learn to speak, to count, to name
things properly. Master these glyphs, and you shall read the world itself.'""",
                "topics": [t[0] for t in by_act.get(1, [])]
            },
            2: {
                "title": "Act II: The Tome of Collections",
                "intro": """
═══════════════════════════════════════════════════════════════════════════
                    THE TOME OF COLLECTIONS AND STRINGS
═══════════════════════════════════════════════════════════════════════════

You've mastered the basic glyphs. Elder Willowbyte leads you deeper into
the Whispering Archive, where ancient tomes float in mid-air, their pages
turning with invisible winds.

'The Language of Nature,' Willowbyte explains, pulling down a leather-bound
volume, 'is not just about numbers and names. It's about COLLECTIONS -
gathering knowledge, organizing information, weaving text into tapestries
of meaning.'

Glowing strings of text spiral around you, forming words and sentences:

'Strings are the threads that bind spells together. Lists are the scrolls
that hold multiple items. Dictionaries map one truth to another. The Cult
cannot corrupt what is well-organized and clearly understood.'

The elder's eyes glow with ancient wisdom:

'Learn to manipulate text, to store collections of data, to organize
your knowledge. These skills will let you READ the corrupted code and
REWRITE it into harmony. The Tome of Collections awaits...'""",
                "topics": [t[0] for t in by_act.get(2, [])]
            },
            3: {
                "title": "Act III: The Branching Paths",
                "intro": """
═══════════════════════════════════════════════════════════════════════════
                        THE BRANCHING PATHS OF LOGIC
═══════════════════════════════════════════════════════════════════════════

The forest path ahead splits into countless directions. Glowing signs hover
at each fork, asking questions: "Is health > 0?", "Does name == 'Grixle'?",
"While enemies remain...". You've entered the Maze of Conditionals.

Elder Willowbyte appears, their form flickering like a loop animation:

'The Cult's first line of defense,' they warn. 'Their technomancers have
woven BRANCHING PATHS throughout the corrupted forest. Code that asks
questions and makes decisions. Programs that loop endlessly, trapping
travelers in infinite recursion.'

A hooded cultist emerges from shadow, their robes covered in tangled
if-statements and while-loops:

'You cannot pass without understanding CONTROL FLOW. Every program must
make decisions. Every spell must sometimes repeat. Will you go left or
right? Will you attack or defend? How many times must you try?'

Willowbyte's voice cuts through the chaos:

'Learn to CONTROL THE FLOW of execution, young one. Master conditionals,
loops, and logical operators. The path that seems to branch infinitely
can be navigated by one who understands the logic of nature itself.'""",
                "topics": [t[0] for t in by_act.get(3, [])]
            },
            4: {
                "title": "Act IV: The Art of Incantations",
                "intro": """
═══════════════════════════════════════════════════════════════════════════
                        THE ART OF INCANTATIONS (FUNCTIONS)
═══════════════════════════════════════════════════════════════════════════

You emerge from the Maze of Conditionals, battle-worn. The corruption grows
stronger. Ahead lies the Citadel of Repeated Patterns, where the same
mistakes echo endlessly because no one learned to write functions.

Elder Willowbyte materializes beside you, holding a staff inscribed with
function signatures:

'Young Grixle, you have been writing code from top to bottom, line by line.
But the ancient masters of Python knew a secret: INCANTATIONS - reusable
spells that can be invoked by name. We call them... functions.'

The staff glows, projecting examples into the air:

    def heal(target):
        target.health += 20
        return target

    def calculate_damage(strength, weapon):
        return strength * weapon.power

'See how the same spell can be cast many times, with different inputs?
This is the Art of Incantations. Write once, invoke forever. The Cult
fears this knowledge because functions bring ORDER to chaos, REUSE to
redundancy.'

'Master functions, and you can write your own restoration spells. You'll
need them for what comes next...'""",
                "topics": [t[0] for t in by_act.get(4, [])]
            },
            5: {
                "title": "Act V: The Scrolls and Grimoires",
                "intro": """
═══════════════════════════════════════════════════════════════════════════
                    THE SCROLLS AND GRIMOIRES OF PERSISTENCE
═══════════════════════════════════════════════════════════════════════════

The Obsidian Library looms before you - a vast archive carved into a
mountainside. Here, knowledge is not just spoken or remembered; it is
WRITTEN and PRESERVED. Forever.

Elder Willowbyte guides you through towering shelves of glowing files:

'All your learning so far,' the elder explains, 'exists only in memory.
When the program ends, it vanishes like morning mist. But the ancient
Language of Nature includes powerful magic for PERSISTENCE - the ability
to write to scrolls (files), to organize grimoires (modules), and to
handle errors when things go wrong.'

Stone tablets float past, inscribed with sacred text:

    with open('spell_book.txt', 'r') as scroll:
        ancient_knowledge = scroll.read()

'The Cult has been corrupting our sacred archives, replacing true knowledge
with exceptions and errors. You must learn to READ files, WRITE files,
IMPORT modules, and HANDLE exceptions gracefully.'

Thunder crashes outside.

'Knowledge saved is power preserved. Master these scrolls, and no corruption
can delete what you've learned. The Iron Wyrm's fortress lies ahead...'""",
                "topics": [t[0] for t in by_act.get(5, [])]
            },
            6: {
                "title": "Act VI: The Living Constructs",
                "intro": """
═══════════════════════════════════════════════════════════════════════════
                        THE LIVING CONSTRUCTS (OBJECTS)
═══════════════════════════════════════════════════════════════════════════

The Iron Fortress rises before you - a twisted structure of metal and
corrupted code. Its walls are alive, shifting, thinking. They are not
mere data; they are OBJECTS - entities with both attributes and behaviors.

Elder Willowbyte's form shimmers, revealing their true nature:

'I have a confession, young Grixle. I am not just a treant. I am a CLASS -
a blueprint. Every time you see me, you're interacting with an INSTANCE
of the Treant class. I have attributes: health, wisdom, age. I have methods:
teach(), guide(), protect().'

The revelation hits you:

'Everything in Python - and in nature - is an OBJECT. Strings are objects.
Lists are objects. Functions are objects. Even you, Grixle, are an instance
of the Hero class!'

The fortress gates pulse with OOP energy:
...
'The Cult has mastered Object-Oriented Programming. Their fortress is built
from classes that inherit from dark base classes. To breach it, you must
understand CLASSES, INSTANCES, INHERITANCE, and COMPOSITION.'

'Become the object you were always meant to be...'""",
                "topics": [t[0] for t in by_act.get(6, [])]
            },
            7: {
                "title": "Act VII: The Grand Algorithm",
                "intro": """
═══════════════════════════════════════════════════════════════════════════
                        THE GRAND ALGORITHM - FINAL BATTLE
═══════════════════════════════════════════════════════════════════════════

The throne room. At last.

The Iron Wyrm coils before you - a dragon made of pure algorithmic
complexity. Its scales are recursive functions, its breath is O(n²) loops,
its roar is the sound of stack overflow errors.

'Foolish child,' the Wyrm's voice rumbles like grinding gears, 'you have
learned the LANGUAGE OF NATURE, but you do not understand EFFICIENCY.
Watch as I sort this array...'

The dragon breathes a bubble sort across the chamber - a pathetically slow
O(n²) algorithm. The room fills with wasted computational cycles.

Elder Willowbyte appears one final time:

'This is it, Grixle. The Wyrm is powerful but INEFFICIENT. It wastes
resources, uses poor algorithms, creates computational chaos. You must
defeat it with OPTIMIZED CODE.'

'Learn the art of sorting: Selection Sort, Insertion Sort, Quicksort,
Merge Sort. Understand Big O notation. Measure complexity. Optimize
ruthlessly.'

Lightning crackles as the Wyrm prepares its final attack:

'I will sort this list of one million elements with BUBBLE SORT! You'll
wait here for HOURS!'

The elder's final words echo:

'Show it the power of Merge Sort, young one. Show it that O(n log n)
defeats O(n²). Restore balance to Fraylon. Make Python elegant again!'
